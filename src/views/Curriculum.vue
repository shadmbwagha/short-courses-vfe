    ...
    <table class="w-full table-fixed even:bg-gray-">
        <thead>
            <tr class="bg-gray-100">
                ...
                <th class="w-1/2 px-6 py-4 text-sm font-medium text-left text-gray-900 uppercase">Curriculum Name</th>
                <th class="w-1/6 px-6 py-4 text-sm font-medium text-left text-gray-900 uppercase">Status</th>
                <th class="w-1/5 px-6 py-4 text-sm font-medium text-left text-gray-900 uppercase">Action</th>
            </tr>
        </thead>
        <tbody class="bg-white">
            <tr v-for="(curriculum,i) in curricula" :key="curriculum.id">
              
                <td class="px-6 py-4 text-sm border-b border-gray-200">{{ i+1 }}</td>
                <td class="px-6 py-4 text-sm truncate border-b border-gray-200 hover:text-blue-400 hover:underline"><router-link :to="{ name: 'curriculumModules', params: { id: curriculum.id}}">{{ curriculum.attributes.name }}</router-link></td>
                <td class="px-6 py-4 text-sm border-b border-gray-200" >
                    <span class="px-2 py-1 text-xs font-medium text-white bg-green-500 rounded-full">Active</span>
                </td>
                <td class="px-6 py-4 text-sm font-medium border-b border-gray-200">
                  <div class="inline-flex items-center rounded-md shadow-sm">
                    ...
                    <button class="inline-flex items-center px-2 py-2 space-x-1 text-sm font-medium bg-white border rounded-r-lg text-slate-800 hover:text-blue-600 hover:bg-slate-100 border-slate-200" @click="deleteCurriculum(curriculum.id)">
                      <span>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                          stroke="currentColor" class="w-3 h-3">
                          <path stroke-linecap="round" stroke-linejoin="round"
